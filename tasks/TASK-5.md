...
8. **Проверьте руками:**
   ...
   Остановятся оба экземпляра. Через минуту с небольшим (`for: 1m` у `AppDown` плюс `group_wait`) бот пришлёт сообщение. Сделайте скриншот. Затем `docker compose start app` — придёт сообщение о разрешении алерта.
